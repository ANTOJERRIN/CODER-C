...
int main()//main function
{
    ...
    printf("standard deviation is=%.2f\n",sd);
     if(sl==INT_MIN)//condition to check if second largest element is not present
            {
                printf("second largest element is not present\n");
            
            }   
            else{
                printf("second largest element is=%d\n",sl);//printing second largest element
            }
    return 0;
